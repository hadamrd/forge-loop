...
def enable_automerge_for_repaired_prs(
    ...
    for outcome in outcomes:
        ...
        # signal to hold a PR back — NOT the presence of unresolved review
        # threads. The old thread gate also blocked APPROVED PRs whose only open
        # threads were leftover sev3 critic inline comments, which (together
        # with the selector bug) produced the #229 multi-hour stall. A
        # critic-approved PR keeps its sev3 threads open forever, so gating on
        # them never lets it land. GitHub's own branch protection still gates
        # the actual merge once auto-merge is enabled.
        if outcome.error:
            append_event(
                cfg.events_file,
