...
class AsyncOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        self.po_ready: asyncio.Queue[tuple[Issue, POResult]] = asyncio.Queue(maxsize=caps.po_ready)
    ...
    def start(self) -> None:
        ...
        for slot in range(self.pools.po):
            self._tasks.append(asyncio.create_task(self._po_worker(slot), name=f"po-{slot}"))
        for slot in range(self.pools.worker):
            self._tasks.append(
                asyncio.create_task(self._worker_worker(slot), name=f"worker-{slot}")
            )
        for slot in range(self.pools.critic):
            self._tasks.append(
                asyncio.create_task(self._critic_worker(slot), name=f"critic-{slot}")
            )
    ...
    async def _po_worker(self, slot: int) -> None:
        while True:
            ...
            try:
                try:
                    po_result = await asyncio.wait_for(self.po_fn(issue), timeout=self.po_timeout_s)
                except TimeoutError:
        ...
        while True:
            ...
            try:
                ...
                if wr.get("pr_url") and wr.get("status") in {"open", "merged"}:
                    await _put_with_overflow(
                        self.pr_pending_critic,
                        wr,
                        stage="pr_pending_critic",
                        emit=self.emit,
                        stats=self.stats.critic,
                    )
    ...
    async def _critic_worker(self, slot: int) -> None:
        while True:
            ...
            try:
                try:
                    cr = await asyncio.wait_for(self.critic_fn(wr), timeout=self.critic_timeout_s)
                except TimeoutError:
