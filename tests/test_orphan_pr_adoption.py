...
def test_recover_pr_url_from_sprint_events(tmp_path: Path) -> None:
    wt = tmp_path / "wt"
    wt.mkdir()
    (wt / "sprint-events.jsonl").write_text(
        json.dumps({"kind": "pr_opened", "pr": PR}) + "\n"
    )
...
def test_selector_returns_only_clean_adoptable_prs(tmp_path: Path) -> None:
    ...
    prs = [
        ...
        {"number": 2, "url": "u2", "headRefName": "loop/2-b",
        ...
        {"number": 6, "url": "u6", "headRefName": "loop/6-e",
         "labels": [{"name": "loop:adopted"}]},
    ]
    ...
    result = orphaned_clean_pr_adoptions(
        cfg, open_prs_fn=open_prs_fn, fetch_issue_fn=fetch_issue_fn,
    )
